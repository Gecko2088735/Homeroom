...
export async function fetchAllClassroomData() {
    const run = async (token) => {
        ...
        for (const course of courses) {
            try {
                courseworkByCourse[course.id] = await listAll(`courses/${course.id}/courseWork`, token, 'courseWork', {
                    pageSize: '100'
                });
            } catch (error) {
                // A course can deny coursework access (e.g. teacher-only items) — skip it, keep the rest.
                if (error.status === 403) courseworkByCourse[course.id] = [];
                else throw error;
            }
        }
        return { courses, courseworkByCourse };
    };
