...
// silent: for automatic background triggers (a timer, a nav click) — reuse an already-cached
// token from an earlier interactive sign-in, but never pop up a Google prompt on their own.
// Returns null (rather than requesting a new token) when there's nothing to reuse.
...
// Returns null (instead of throwing) for a silent call with nothing to reuse — the caller treats
// that as "not connected right now," not an error.
export async function fetchAllClassroomData({ silent = false } = {}) {
    ...
    try {
        ...
        clearToken();
        const retryToken = await getAccessToken({ silent });
